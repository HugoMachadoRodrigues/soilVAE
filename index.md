...
- a supervised head $\widehat{y} = f_{\psi}(z)$ predicting the property

### Objective (supervised $\beta$-VAE)

We minimize a weighted sum:

$$\mathcal{L}(x,y) = \underset{\text{reconstruction}}{\underbrace{\parallel x - \widehat{x} \parallel_{2}^{2}}}\; + \;\beta\;\underset{\text{regularization}}{\underbrace{D_{KL}\!\left( q_{\phi}(z \mid x)\, \parallel \,\mathcal{N}(0,I) \right)}}\; + \;\alpha\;\underset{\text{regression}}{\underbrace{\parallel y - \widehat{y} \parallel_{2}^{2}}}.$$
